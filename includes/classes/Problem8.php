<?php
/**
 * Project Euler - Problem 8
 *
 * Largest product in a series
 *
 * Find the greatest product of five consecutive digits in the 1000-digit number.
 *
 * @category ProjectEuler
 * @package Problem8
 * @author Michael Reeves <mike.reeves@gmail.com>
 * @link http://projecteuler.net/problem=8
 *
 */
class Problem8 extends Problem_Abstract
{
    /**
     * Our input "number". We have to represent this as a string because an integer this big would overflow
     * @const string 1000 digit "number"
     */
    const INPUT = '';

    /**
     * Wrapper method to output our answer with the appropriate input variables
     *
     * @return int
     */
    public function execute()
    {
        return $this->findLargestProductInASeries(self::INPUT);
    }

    /**
     * Find the greatest product of five consecutive digits in the 1000-digit number.
     *
     * @param string $series long series of integers
     *
     * @return int product of 5 consecutive digits
     */
    private function findLargestProductInASeries($series){
        $numArray = str_split($series);
        $answers = array(); // key is sequence of #s, value is product of #s
        $iMax = count($numArray) - 5;
        for ($i = 0; $i <= $iMax; $i++){
            $testSequence = array(
                                $numArray[$i],
                                $numArray[$i+1],
                                $numArray[$i+2],
                                $numArray[$i+3],
                                $numArray[$i+4]
                            );
            $answers[implode('',$testSequence)] = array_product($testSequence);
        }
        arsort($answers); //sort by product (array value) descending
        $finalAnswer = array_shift($answers); // first value in array
        return $finalAnswer;
    }
}
